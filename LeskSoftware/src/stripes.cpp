...
Stripes::Stripes(int _port, int _length, int _direction, int _speed, short int _mode, uint32_t _color)
...
void Stripes::setEffect(uint8_t effect) {
    ws2812fx.setMode(effect);
    ws2812fx.service();
}

void Stripes::setBrightness(uint8_t brightness) {
    ws2812fx.setBrightness(brightness);
    ws2812fx.service();

}

void Stripes::setColor(uint8_t g, uint8_t r, uint8_t b) {
